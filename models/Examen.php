...
class Examen extends \yii\db\ActiveRecord
{
    ...
    {
        return $this->hasMany(ExamenParametro::className(), ['examen_id' => 'id']);
    ...
    {
        return $this->hasMany(ExamenGermen::className(), ['examen_id' => 'id']);        
    ...
    public function getForm() {       
       ...
       foreach ($this->analisis->parametrosParaIngresoResultado as $param) {
            ...
            if(isset($param->seccion_id) && $seccion != $param->seccion_id ){
                $form.=Html::tag(
                    'div',Html::tag(
                        'div',Html::tag('div',$param->seccion->descripcion,['class'=>'btn btn-info']),['class'=>'col-md-9'])
                    ,['class'=>'row']);
            ...
            $referencia = Examen::generarReferencia($param);
            //si es referencia selecionable
            $referenciaInput='';
            $referenciaHiddenInput='';
            if( trim( $param->valores_referencia_seleccionable ) !='' ){
                    $values = [];
