...
class PDF_ORDEN_ACCESS extends FPDF
{
        ...
        if( !empty($this->orden->laboratorista_id ) && $this->orden->laboratorista_id <> 2 ){
            ...
            if( $this->digital_sign){
                if( strlen($laboratorista->firma_digital_fullname) > 0) {
                    $url = utf8_encode("FIRMADO POR: " . $laboratorista->firma_digital_fullname .
                    ...
                    $this->MultiCell(105, 4, utf8_decode($this->subjectSignature($laboratorista->firma_digital_fullname)), $this->debug, 'J', 0);
                }
            }else{
        ...
        if( !is_null($responsableTecnico)){
            ...
            if( $this->digital_sign){
                if( strlen($responsableTecnico->firma_digital_fullname) > 0) {
                    $url = utf8_encode("FIRMADO POR: " . $responsableTecnico->firma_digital_fullname .
                        chr(10) . "FECHA: " . $this->orden->fecha_firmado_digital);
                    $qrCode = (new QrCode($url));
                    $qrCode->setSize(250)->setMargin(5)->useForegroundColor(55, 55, 55);
                    ...
                    $this->SetFont($this->font_sign, 'B', $this->font_title_size);
                    $this->SetXY(140, $y - 14);
                    $this->MultiCell(105, 4, utf8_decode($this->subjectSignature($responsableTecnico->firma_digital_fullname)), $this->debug, 'J', 0);
                }
            }
            else {
    ...
    private function analisisAccessLab(){
           $this->SetX($this->line_begin+5);
		     $this->SetFont($this->font,'B',$this->font_body_size);
           $this->Cell(85,5, utf8_decode('MÉTODO:'),$this->debug, 0,'L',0);
           $this->ln();
           $this->SetX($this->line_begin+10);
		     $this->SetFont($this->font,'',$this->font_body_size);
           $this->Cell(165,5, utf8_decode($this->examen->analisis->nombre ),$this->debug, 0,'L',0);
           $this->Ln();
                ...
                if(isset($parametroExamen)){                    
                    if( !empty( $parametroExamen->valor ) or trim($parametroExamen->valor) != '' ){
                        ...
                        if(isset($parametro->metodo_id)){
                            ...
                            $this->SetFont($this->font,'',$this->font_body_size);
                            $this->Cell(165,5, utf8_decode($parametro->metodo->descripcion),$this->debug,0,'L',0);                           
                        }
                        $this->ln(7);                        
                        $this->SetX($this->line_begin+5);
						$this->SetFont($this->font,'B',$this->font_body_size);
                        $this->Cell(85,5, utf8_decode('ENSAYO:'),$this->debug, 0,'L',0);
                        $this->ln();
                        $this->SetX($this->line_begin+10);
						$this->SetFont($this->font,'',$this->font_body_size);
                        $this->MultiCell(165,5, utf8_decode( $parametro->ensayo ),$this->debug ,'J', 0 );
                        $this->ln(7);
                        
                        $this->SetX($this->line_begin+10);
						$this->SetFont($this->font,'B',$this->font_body_size);
                        $this->Cell(85,5, utf8_decode('Amplificación y detección:'),$this->debug, 0,'L',0);
                        $this->ln();
                        $this->SetX($this->line_begin+10);
						$this->SetFont($this->font,'',$this->font_body_size);
                        $this->MultiCell(165,5, utf8_decode(  $parametro->amplificacion_deteccion ),$this->debug ,'J', 0 );
                        $this->ln(7);
						...
						$x = $this->GetX();
						$this->SetFillColor( 253, 210, 195 );
						$this->Cell(165,15,utf8_decode(''),$this->debug,0,'L',true);
						...
						$this->SetXY($x,$y);
                        $this->SetFont($this->font,'B',$this->font_body_size);						
                        $this->Cell(165,5, utf8_decode('Resultados del Análisis'),$this->debug,0,'C',0); 
                        $this->ln();
                        
                        $this->SetX($this->line_begin+10);
                        $this->Cell(165,5, utf8_decode($this->examen->analisis->nombre ),$this->debug, 0,'C',0);
                        $this->ln();
                        
                        $this->SetX($this->line_begin+10);
						$this->SetFont($this->font,'',$this->font_body_size);
                        $this->Cell(75,5, utf8_decode($parametro->descripcion ),$this->debug,0,'R',0); 
                        $this->SetX( $this->GetX() + 10 );
						$this->SetFont($this->font,'B',$this->font_body_size);
                        if( strlen( trim( $parametroExamen->medida ) ) == 0 && strlen( trim( $parametroExamen->valor ) ) > 10){                          
                            $this->Cell(80,5, utf8_decode( trim($parametroExamen->valor )),$this->debug ,0,'L', 0 );
                        }else{                          
                            $this->Cell(80,5, utf8_decode( trim($parametroExamen->valor.' '.$parametroExamen->medida )),$this->debug ,0,'L', 0 );                           
                        }                        
                        $this->ln(10);
                        
                        $this->SetX($this->line_begin+10);
						$this->SetFont($this->font,'B',$this->font_body_size);
                        $this->Cell(165,5, utf8_decode( 'Intervalos de referencias:'),$this->debug, 0,'L',0);
                        $this->ln();
                        
                        $this->SetX($this->line_begin+10);
						$this->SetFont($this->font,'',$this->font_body_size);
                        $this->MultiCell(165,5, utf8_decode( $parametroExamen->referencia ),$this->debug ,'J', 0 );
                        $this->ln();
						
						if( strlen ( trim($parametro->limite_deteccion) ) > 0 ){
							$this->SetX($this->line_begin+10);
							$this->SetFont($this->font,'B',$this->font_body_size);
							$this->Cell(70,5, utf8_decode( 'Límite de detección:'),$this->debug, 0,'L',0);					
							$this->SetFont($this->font,'',$this->font_body_size);						
							$this->Cell(95,5, utf8_decode( $parametro->limite_deteccion  ),$this->debug, 0,'L',0);
							$this->ln();
						}
